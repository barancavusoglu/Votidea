...
public class TopVotedCommentedAdapter extends BaseQuickAdapter<GetTopCommentedResponse.TopResultsBean,BaseViewHolder> {

    RequestOptions requestOptions,profilePicRequestOptions;
    public TopVotedCommentedAdapter(int layout, List data){
        super(layout,data);
    }

    @Override
    protected void convert(BaseViewHolder viewHolder, GetTopCommentedResponse.TopResultsBean item) {
        ...
        if (loggedUser.getLoginView().getID() == (item.getVoting().getMasterVotingView().getOwnerUserID())){
        ...
        if (item.getVoting().getMasterVotingView().getDescription().isEmpty())
        {
            tvDesc.setVisibility(View.GONE);
        }

        if (!item.getVoting().getMasterVotingView().isIsCommentAllowed()){
        ...
        else
        {
            ...
            if(item.getVoting().getMasterVotingView().getCommentCount() == 0)
            {
                btComments.setText(mContext.getResources().getString(R.string.thereAreNoComments));
            }
            else{
                btComments.setText(mContext.getResources().getString(R.string.seeComments) + " (" +item.getVoting().getMasterVotingView().getCommentCount()+")");
            }
        }

        try{
            if(item.getVoting().getMasterVotingView().isIsAnonymous()){
               ...
               Glide.with(mContext).load(R.drawable.com_facebook_profile_picture_blank_square).apply(profilePicRequestOptions).into((ImageView) viewHolder.getView(R.id.ivProfilePic));
                ...
                viewHolder.setText(R.id.tvNameSurname,item.getVoting().getMasterVotingView().getOwnerUsername());
                Glide.with(mContext).load(item.getVoting().getMasterVotingView().getOwnerPicURL()).apply(profilePicRequestOptions).into((ImageView) viewHolder.getView(R.id.ivProfilePic));
            }

            if (item.getVoting().getMasterVotingView().getChoiceCount() == 2){
                Glide.with(mContext).load(item.getVoting().getChoiceList().get(0).getPictureURL()).apply(requestOptions).into(iv1);
                Glide.with(mContext).load(item.getVoting().getChoiceList().get(1).getPictureURL()).apply(requestOptions).into(iv2);

                voteCount = item.getVoting().getChoiceList().get(0).getClickedCount() + item.getVoting().getChoiceList().get(1).getClickedCount();

                iv3.setVisibility(View.GONE);
                iv4.setVisibility(View.GONE);

                int iv1Percentage = (int)item.getVoting().getChoiceList().get(0).getPercentage();
                int iv2Percentage = (int)item.getVoting().getChoiceList().get(1).getPercentage();
            ...
            else if(item.getVoting().getMasterVotingView().getChoiceCount() == 3) {
                Glide.with(mContext).load(item.getVoting().getChoiceList().get(0).getPictureURL()).apply(requestOptions).into(iv1);
                Glide.with(mContext).load(item.getVoting().getChoiceList().get(1).getPictureURL()).apply(requestOptions).into(iv2);
                Glide.with(mContext).load(item.getVoting().getChoiceList().get(2).getPictureURL()).apply(requestOptions).into(iv3);

                voteCount = item.getVoting().getChoiceList().get(0).getClickedCount() + item.getVoting().getChoiceList().get(1).getClickedCount() + item.getVoting().getChoiceList().get(2).getClickedCount();
                ...
                int iv1Percentage = (int)item.getVoting().getChoiceList().get(0).getPercentage();
                int iv2Percentage = (int)item.getVoting().getChoiceList().get(1).getPercentage();
                int iv3Percentage = (int)item.getVoting().getChoiceList().get(2).getPercentage();
                ...
                Glide.with(mContext).load(item.getVoting().getChoiceList().get(1).getPictureURL()).apply(requestOptions).into(iv2);
                Glide.with(mContext).load(item.getVoting().getChoiceList().get(2).getPictureURL()).apply(requestOptions).into(iv3);
                Glide.with(mContext).load(item.getVoting().getChoiceList().get(3).getPictureURL()).apply(requestOptions).into(iv4);

                voteCount = item.getVoting().getChoiceList().get(0).getClickedCount() + item.getVoting().getChoiceList().get(1).getClickedCount() + item.getVoting().getChoiceList().get(2).getClickedCount() + item.getVoting().getChoiceList().get(3).getClickedCount();
                ...
                int iv2Percentage = (int)item.getVoting().getChoiceList().get(1).getPercentage();
                int iv3Percentage = (int)item.getVoting().getChoiceList().get(2).getPercentage();
                int iv4Percentage = (int)item.getVoting().getChoiceList().get(3).getPercentage();
            ...
            Glide.with(mContext).load(mContext.getResources().getIdentifier(item.getVoting().getMasterVotingView().getFlagCode(), "drawable", mContext.getPackageName())).into((ImageView) viewHolder.getView(R.id.ivCountry));
        }
        ...
        if (item.getVoting().getSelectedChoice() == 0 && item.getVoting().getMasterVotingView().getOwnerUserID() != loggedUser.getLoginView().getID()){// oylama yapılmadıysa
        ...
        else if(item.getVoting().getSelectedChoice() == 1){
            ivLike1.setVisibility(View.VISIBLE);
            ivLike2.setVisibility(View.GONE);
            ivLike3.setVisibility(View.GONE);
            ivLike4.setVisibility(View.GONE);
        }
        else if(item.getVoting().getSelectedChoice() == 2){
            ivLike1.setVisibility(View.GONE);
            ivLike2.setVisibility(View.VISIBLE);
            ivLike3.setVisibility(View.GONE);
            ivLike4.setVisibility(View.GONE);
        }
        else if(item.getVoting().getSelectedChoice() == 3){
            ivLike1.setVisibility(View.GONE);
            ivLike2.setVisibility(View.GONE);
            ivLike3.setVisibility(View.VISIBLE);
            ivLike4.setVisibility(View.GONE);
        }
        else if(item.getVoting().getSelectedChoice() == 4){
            ivLike1.setVisibility(View.GONE);
            ivLike2.setVisibility(View.GONE);
            ivLike3.setVisibility(View.GONE);
        ...
        try {
            ...
            Date myDate = simpleDateFormat1.parse(item.getVoting().getMasterVotingView().getCreationDate());
        ...
        viewHolder.setText(R.id.tvDesc,item.getVoting().getMasterVotingView().getDescription());
    }
}